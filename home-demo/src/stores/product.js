...
export const useProductStore = defineStore('product', {
  ...
  actions: {
    ...
    async fetchProducts() {
      ...
      try {
        console.log('调用API获取商品列表...');
        const products = await productApi.getAllProducts();
        console.log('从API获取的原始商品数据:', products);
        
        // 检查数据格式
        if (!Array.isArray(products)) {
          console.error('API返回的不是数组:', products);
          
          // 尝试从响应中提取数据
          let productsArray = [];
          if (products && products.content && Array.isArray(products.content)) {
            console.log('从content字段提取数组数据');
            productsArray = products.content;
          ...
          if (productsArray.length === 0) {
            console.warn('无法从响应中提取商品数组，使用空数组');
          }
          
          // 处理返回的产品数据
          this.products = productsArray.map(p => this.normalizeProduct(p));
        } else {
          // 直接处理返回的数组数据
          this.products = products.map(p => this.normalizeProduct(p));
        }
        
        console.log('规范化后的商品数据:', this.products);
        return { success: true };
      } catch (error) {
